"""Build, store, and retrieve feature tables — versioned, idempotent, leak-free.

Features are computed over the **full** continuous history of each symbol (so the
EWM/rolling state is correctly seeded and the value of a feature at ``t`` never
depends on where a build window starts), then written to the lake at::

    data/features/symbol=<SYM>/date=<YYYY-MM-DD>/part-<uuid>.parquet

partitioned, zstd, idempotent on the natural key ``ts_event``. Re-running never
duplicates. ``--start/--end`` bound only what is *written*, never what is
computed, so stored values are identical regardless of the write window.

Retrieval (:func:`read_features`, :func:`features_asof`) goes through the DuckDB
store and its ``asof_join``, preserving the no-look-ahead guarantee end-to-end:
a bar at ``t`` only ever sees a feature row with ``ts_event <= t``.
"""
...
from .compute import compute_features, feature_names
...
def _write_symbol(frame: pl.DataFrame, symbol: str) -> int:
    """Append a symbol's feature frame; idempotent on ts_event per date partition."""
    if frame.is_empty():
        return 0
...
def build_features(
    ...
    *,
    ...
    """Compute features over full history for each symbol and write the lake tables.
    ...
    try:
        cont = {s: _continuous(store, s) for s in symbols}
        pair = cfg.cross_asset.pair
        ts_ingest = datetime.now(UTC)
        result: dict[str, int] = {}
        for sym in symbols:
            other_sym = pair[1] if sym == pair[0] else pair[0]
            other = cont.get(other_sym)
            feats = compute_features(cont[sym], cfg, symbol=sym, other=other)
...
def read_features(
    ...
    """Feature rows for ``symbol`` in ``[start, end]`` (inclusive, UTC), latest-ingest wins."""
    own = store is None
    store = store or DuckStore()
    try:
...
def features_asof(
    bars: pl.DataFrame, symbol: str, *, store: DuckStore | None = None
) -> pl.DataFrame:
    """Attach, to each row of ``bars`` (must have ``ts_event``), the latest feature
    ...
    try:
        ...
        feats = read_features(symbol, lo, hi, store=store)
        if feats.is_empty():
            return bars
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(prog="features.build", description=__doc__)
